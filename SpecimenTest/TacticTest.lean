...
-- Test 5: several hypotheses constraining the same variable
/-- tests passed -/
#guard_msgs (info, substring := true) in
...
-- Test 8: the `specimen` tactic on a true property. It is a diagnostic and leaves the goal
-- open, so `sorry` closes it; the assertion is on the reported test count.
/-- info: 100 tests passed (0 discarded) -/
...
--
-- Run with `tests := 0`, so this covers derivation and codegen only and asserts nothing about
-- the property. Generating `e` and `τ` jointly as outputs of `typing` and then requiring `e`
-- to step is too constrained for the current schedule: measured over 200 tests at sizes 2-8,
-- 199 were discarded and none reached the bug. Raise `tests` once that mode improves.
