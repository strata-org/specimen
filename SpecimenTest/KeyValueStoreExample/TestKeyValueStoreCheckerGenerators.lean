...
set_option specimen.scoreType "Scoring.InputAwareGradedScore"
...
-- #guard_msgs(drop info) in
#eval Gen.run (ArbitrarySizedSuchThat.arbitrarySizedST (fun crs => KeyValueStore.EvalApiCalls (0, []) crs) 11) 20

private def isFailure : Result → Bool
  | .OpResult .NoSuchKeyFailure => true
  | .OpResult .NoSuchVersionFailure => true
  | .OpResult .NoSuchKeyResult => true
  | .Error _ => true
  | _ => false

/-- Generate a single successful API call from a given state, retrying until no failure result.
    Falls back to CreateBucket if retries are exhausted. -/
def genSuccessfulApiCall (state : Nat × List (Nat × List (String × String)))
    : Gen (APICall × Result × (Nat × List (Nat × List (String × String)))) := do
  let gen := ArbitrarySizedSuchThat.arbitrarySizedST
    (fun crns => KeyValueStore.EvalApiCall state crns) 10
  for _ in List.range 30 do
    let result ← gen
    if !isFailure result.2.1 then
      return result
  -- Fallback: always-valid CreateBucket
  let n := state.1
  return (.CreateBucket, .Created n, (n + 1, (n, []) :: state.2))

/-- Generate a Set operation on a specific bucket (always succeeds). -/
def genSetOnBucket (n : Nat) (bucketState : List (String × String))
    (store : Nat × List (Nat × List (String × String)))
    : Gen (APICall × Result × (Nat × List (Nat × List (String × String)))) := do
  let k ← instKeyValueStoreArbitraryString.arbitrary
  let v ← instKeyValueStoreArbitraryString.arbitrary
  let newBucketState := (k, v) :: bucketState
  let newStore := store.2.map fun (id, s) => if id == n then (id, newBucketState) else (id, s)
  return (.OpBucket n (.Set k v), .OpResult .Ok, (store.1, newStore))

/-- Generate a list of successful API calls from an initial state.
    After creating a bucket, immediately seeds it with a Set so that
    subsequent operations on non-empty state can succeed. -/
def genSuccessfulApiCalls (initState : Nat × List (Nat × List (String × String)))
    (len : Nat) : Gen (List (APICall × Result) × (Nat × List (Nat × List (String × String)))) := do
  let mut state := initState
  let mut trace : List (APICall × Result) := []
  let mut remaining := len
  while remaining > 0 do
    remaining := remaining - 1
    let (call, result, newState) ← genSuccessfulApiCall state
    trace := trace ++ [(call, result)]
    state := newState
    -- After creating a bucket, seed it with a Set so ops can succeed on it
    match call, result with
    | .CreateBucket, .Created bucketId =>
      if remaining > 0 then
        remaining := remaining - 1
        let (setCall, setResult, setState) ← genSetOnBucket bucketId [] state
        trace := trace ++ [(setCall, setResult)]
        state := setState
    | _, _ => pure ()
  return (trace, state)

#eval Gen.run (genSuccessfulApiCalls (0, []) 15) 20

-----------------------------------------------------------------------
-- Wishlist-based generation: derive generator for ExecWishList
-----------------------------------------------------------------------

set_option specimen.autoDeriveDeps true
set_option specimen.multiOutput true
set_option specimen.scoreType "Scoring.InputAwareGradedScore"
derive_mutual (fun s => ∃ (ops : List WishOp) (o : List (StateAPICall × StateResult) × List (String × String)), KeyValueStore.ExecWishList s ops o)

-- Test: generate a sequence of successful ops on an initially-empty bucket
#eval Gen.run (ArbitrarySizedSuchThat.arbitrarySizedST
  (fun (bc : List WishOp × (List (StateAPICall × StateResult) × List (String × String))) =>
    KeyValueStore.ExecWishList [] bc.1 bc.2) 20) 20
#eval Gen.run (ArbitrarySizedSuchThat.arbitrarySizedST
  (fun (bc : List WishOp × (List (StateAPICall × StateResult) × List (String × String))) =>
    KeyValueStore.ExecWishList [] bc.1 bc.2) 20) 42
